...
const BORDER='rgba(201,146,74,0.12)',MUTED='#6A6560',GREEN='#4A9C7A',RED='#C84B31'
...
const EL_MODELS=[
  ...
  {id:'eleven_flash_v2_5',label:'Flash v2.5 — Ultra-low latency (75ms)'},
  {id:'eleven_turbo_v2_5',label:'Turbo v2.5 — Balanced (250ms)'},
...
const BLANK={
  ...
  aiimagemodel:'',aigeneratedprompt:'',soundaimodel:'eleven_multilingual_v2',
...
// ── Voice Library ─────────────────────────────────────────────────────────────
function VoiceLibrary({onSelect,onClose}){
  ...
  useEffect(()=>{
    fetch('/api/elevenlabs-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'list_voices'})})
      .then(r=>r.json()).then(d=>{setVoices(d.voices||[]);setLoading(false)}).catch(()=>setLoading(false))
  },[])
  const filtered=voices.filter(v=>!search||v.name?.toLowerCase().includes(search.toLowerCase())||(v.labels?.description||'').toLowerCase().includes(search.toLowerCase())||(v.category||'').toLowerCase().includes(search.toLowerCase()))
  ...
  return(
    <div style={{position:'fixed',top:0,right:0,bottom:0,width:'400px',background:SURFACE,borderLeft:`1px solid ${BORDER}`,zIndex:200,display:'flex',flexDirection:'column',boxShadow:'-8px 0 32px rgba(0,0,0,0.6)'}}>
      <div style={{padding:'14px 20px',borderBottom:`1px solid ${BORDER}`,display:'flex',alignItems:'center',gap:'10px'}}>
        <div style={{fontSize:'0.68rem',color:GOLD,letterSpacing:'0.14em',textTransform:'uppercase',flex:1}}>Voice Library</div>
      ...
      <div style={{flex:1,overflowY:'auto'}}>
        ...
        {!loading&&filtered.length===0&&<div style={{padding:'24px',color:MUTED,fontSize:'0.78rem',textAlign:'center'}}>No voices found</div>}
        {filtered.map(v=>(
          <div key={v.voice_id} style={{padding:'12px 20px',borderBottom:`1px solid rgba(201,146,74,0.06)`,display:'flex',alignItems:'center',gap:'10px'}}
            onMouseEnter={e=>e.currentTarget.style.background='rgba(201,146,74,0.03)'}
            onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:'0.85rem',color:CREAM,marginBottom:'2px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{v.name}</div>
              <div style={{fontSize:'0.65rem',color:MUTED}}>{v.category||''}{v.labels?.accent?` · ${v.labels.accent}`:''}</div>
            </div>
            {v.preview_url&&<button onClick={()=>playPreview(v)} style={{width:'28px',height:'28px',background:playing===v.voice_id?GOLD:'rgba(255,255,255,0.06)',border:`1px solid ${playing===v.voice_id?GOLD:BORDER}`,color:playing===v.voice_id?SURFACE:CREAM,cursor:'pointer',fontSize:'0.6rem',flexShrink:0}}>{playing===v.voice_id?'■':'▶'}</button>}
            <button onClick={()=>onSelect(v.voice_id,v.name)} style={{background:'rgba(201,146,74,0.1)',border:`1px solid rgba(201,146,74,0.25)`,color:GOLD,padding:'5px 10px',cursor:'pointer',fontFamily:'DM Sans, sans-serif',fontSize:'0.63rem',letterSpacing:'0.08em',textTransform:'uppercase',flexShrink:0}}>Use</button>
          </div>
        ))}
      </div>
    </div>
...
function VoiceTab({data,onChange,locked,assetMeta,onVoiceIdChange,savedAssetId}){
  ...
  const [previewText,setPreviewText]=useState("Hello, my name is the character and this is a preview of my voice. I am here to help tell the story in this production.")
  ...
  async function doGeneratePrompt(){
    ...
    if(parts.length>=4){onChange('voiceprompt',structured);setGenPrompt(false);return}
    try{
      const res=await fetch('/api/score',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:300,messages:[{role:'user',content:`Generate a detailed ElevenLabs Voice Design v3 voice prompt for a character named ${assetMeta.name||'Character'}.\nKnown attributes: ${structured||'none'}\nImage prompt: ${(data.prompt||'').slice(0,200)}\nReturn ONLY the voice prompt, one detailed paragraph.`}]})})
      const d=await res.json()
      onChange('voiceprompt',d.content?.map(b=>b.text||'').join('')||structured)
    }catch(e){onChange('voiceprompt',structured)}
    setGenPrompt(false)
  }

  async function doGeneratePreviews(){
    if(!data.voiceprompt?.trim()){setError('Generate a Voice Prompt first.');return}
    setGenerating(true);setError('')
    const safeText=previewText.length>=100?previewText:previewText+'  This is a voice preview for the character in the Culmina micro-drama production series.'.slice(0,110-previewText.length+50)
    const newDrafts=[]
    for(let i=0;i<numPrev;i++){
      try{
        const r=await fetch('/api/elevenlabs-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'design',voice_description:data.voiceprompt,text:safeText,gender:data.voicegender?.toLowerCase()||undefined,age:data.voiceage||undefined,loudness:i*0.1,quality:1.0})})
        const d=await r.json()
        if(!r.ok)throw new Error(d.error||'Unknown error')
        newDrafts.push({id:Date.now()+i,num:drafts.length+newDrafts.length+1,generated_voice_id:d.generated_voice_id,audio_base64:d.audio_base64,audio_url:d.audio_url,snippet:(data.voiceprompt||'').slice(0,55),isFinal:false})
  ...
  async function toggleFinal(draft){
    ...
    if(fin?.generated_voice_id){
      try{
        const r=await fetch('/api/elevenlabs-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save_voice',generated_voice_id:fin.generated_voice_id,name:assetMeta.name||`Culmina Voice ${Date.now()}`,description:data.voiceprompt||''})})
        const d=await r.json()
  ...
  const btnBase={fontFamily:'DM Sans, sans-serif',fontSize:'0.72rem',letterSpacing:'0.08em',textTransform:'uppercase',cursor:'pointer',border:'none'}

  return(
    <div style={{padding:'24px 28px 60px'}}>
      {showLib&&<VoiceLibrary onSelect={(id)=>{onVoiceIdChange(id);setShowLib(false)}} onClose={()=>setShowLib(false)}/>}
      ...
      <div style={{marginBottom:'20px'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          ...
          <button onClick={()=>setShowLib(s=>!s)} disabled={locked}
            style={{...btnBase,background:'rgba(201,146,74,0.1)',color:locked?MUTED:GOLD,padding:'9px 14px',border:`1px solid rgba(201,146,74,0.25)`,whiteSpace:'nowrap'}}>
            🔍 Browse
          </button>
          {voiceId&&(
            <button onClick={async()=>{
              try{
                const safe=previewText.length>=100?previewText:previewText+'  This is a voice preview for this character in the Culmina production.'.slice(0,110-previewText.length+50)
                ...
                if(src)new Audio(src).play();else setError(d.error||'No audio')
              }catch(e){setError(e.message)}
            }} style={{...btnBase,background:'rgba(74,156,122,0.1)',color:GREEN,padding:'9px 12px',border:`1px solid rgba(74,156,122,0.25)`,whiteSpace:'nowrap'}}>
              ▶ Test
            </button>
          )}
        </div>
      ...
      {/* Sliders */}
      <Sec title="Voice Settings" open={true}>
        <div style={{paddingTop:'10px'}}>
          <Slider label="Speed" value={data.voicespeed??1.0} onChange={v=>onChange('voicespeed',v)} min={0.5} max={2.0} step={0.05} disabled={locked}
            note="Most natural conversations: 0.9–1.1×. Slower for complex topics, faster for routine information."/>
          <Slider label="Stability" value={data.voicestabilityscore??0.5} onChange={v=>onChange('voicestabilityscore',v)} min={0} max={1} step={0.05} disabled={locked}
            note="Lower (0.30–0.50): more emotional and dynamic. Higher (0.60–0.85): consistent but may sound monotonous."/>
          <Slider label="Similarity" value={data.voicesimilarity??0.75} onChange={v=>onChange('voicesimilarity',v)} min={0} max={1} step={0.05} disabled={locked}
            note="Higher values boost clarity and consistency. Very high values may cause audio distortions."/>
          <Slider label="Style Exaggeration" value={data.voicestyle??0.0} onChange={v=>onChange('voicestyle',v)} min={0} max={1} step={0.05} disabled={locked}
            note="Amplifies the style of the voice. Increases latency when set above 0."/>
        </div>
      </Sec>

      {/* Voice Characteristics */}
      <Sec title="Voice Characteristics" open={true}>
        <div style={{paddingTop:'10px'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'12px'}}>
            ...
            <div><div style={lbl}>Accent</div><input value={data.voiceaccent||''} onChange={e=>!locked&&onChange('voiceaccent',e.target.value)} disabled={locked} style={mkI(locked)} placeholder="e.g. Eastern European, RP British"/></div>
      ...
      <div style={{marginTop:'20px',marginBottom:'16px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'8px'}}>
          ...
          <button onClick={doGeneratePrompt} disabled={locked||genPrompt}
            style={{...btnBase,background:'rgba(201,146,74,0.1)',color:locked?MUTED:GOLD,padding:'6px 14px',border:`1px solid rgba(201,146,74,0.25)`}}>
            {genPrompt?'Generating…':'Generate Prompt'}
          </button>
        </div>
        <textarea value={data.voiceprompt||''} onChange={e=>!locked&&onChange('voiceprompt',e.target.value)} disabled={locked}
          style={{...mkT(locked),minHeight:'80px'}} placeholder="ElevenLabs voice design prompt — click Generate Prompt or write manually..."/>
      </div>
      ...
      <div style={{marginBottom:'16px'}}>
        ...
        <div style={{fontSize:'0.62rem',color:MUTED,marginTop:'3px'}}>Minimum 100 characters required by ElevenLabs.</div>
      </div>
      ...
      <div style={{display:'flex',alignItems:'flex-end',gap:'12px',marginBottom:'16px'}}>
        ...
        <button onClick={doGeneratePreviews} disabled={generating||locked||!data.voiceprompt?.trim()}
          style={{...btnBase,background:generating?'rgba(201,146,74,0.4)':GOLD,color:SURFACE,padding:'8px 20px',fontWeight:500,opacity:!data.voiceprompt?.trim()?0.4:1}}>
      ...
      {drafts.length>0&&(
        <div>
          <div style={{fontSize:'0.68rem',color:CHARCOAL,letterSpacing:'0.12em',textTransform:'uppercase',marginBottom:'10px'}}>Voice Drafts — {drafts.length} generated</div>
          <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
            {drafts.map(draft=>{
              const src=draft.audio_url||(draft.audio_base64?`data:audio/mpeg;base64,${draft.audio_base64}`:null)
              return(
                <div key={draft.id} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 14px',border:`2px solid ${draft.isFinal?GREEN:BORDER}`,background:draft.isFinal?'rgba(74,156,122,0.05)':'rgba(255,255,255,0.01)',transition:'border-color 0.15s'}}>
                  ...
                  <div style={{flex:1,minWidth:0,fontSize:'0.72rem',color:CHARCOAL,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{draft.snippet||'Voice design preview'}</div>
                  {src?<audio controls src={src} style={{height:'28px',maxWidth:'180px'}} onClick={e=>e.stopPropagation()}/>:<span style={{fontSize:'0.65rem',color:MUTED}}>No audio</span>}
                  <button onClick={()=>toggleFinal(draft)} style={{...btnBase,background:draft.isFinal?'rgba(74,156,122,0.15)':'none',border:`1px solid ${draft.isFinal?'rgba(74,156,122,0.4)':BORDER}`,color:draft.isFinal?GREEN:CHARCOAL,padding:'4px 10px',flexShrink:0}}>{draft.isFinal?'✓ Final':'Final'}</button>
...
function ImageTab({data,onChange,locked,assetMeta}){
  ...
  const [imgDrafts,setImgDrafts]=useState([]),[finalIdx,setFinalIdx]=useState(null)
  const [genImg,setGenImg]=useState(false),[imgErr,setImgErr]=useState('')

  function autoPrompt(){
    const p=[]
    if(isPerson){if(assetMeta.name)p.push(assetMeta.name);if(data.characterimportance)p.push(data.characterimportance+' character');if(data.sex)p.push(data.sex);if(isChar&&data.ethnicity)p.push(data.ethnicity);if(isChar&&data.skintone)p.push(data.skintone+' skin');if(data.heightft)p.push(`${data.heightft}ft ${data.heightin??0}in`);if(data.haircolor)p.push(`${data.haircolor} ${data.hairlength||''} hair`.trim());if(data.eyecolor)p.push(data.eyecolor+' eyes');if(data.clothingdescription)p.push('Wearing: '+data.clothingdescription)}
    if(isSet){if(assetMeta.name)p.push(assetMeta.name);if(data.timeperiod)p.push(data.timeperiod);if(data.bgimagedesc)p.push(data.bgimagedesc)}
    p.push('Cinematic lighting. 16:9 1080p. Photorealistic.')
    onChange('prompt',p.join(', '))
  }
  ...
  async function genImages(){
    ...
    try{
      const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${mid}:predict?key=${k}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({instances:[{prompt:data.prompt}],parameters:{sampleCount:3,aspectRatio:data.promptaspectratio||'16:9',safetyFilterLevel:'block_some'}})})
      if(!r.ok){const e=await r.json().catch(()=>({}));throw new Error(e?.error?.message||`API ${r.status}`)}
      const j=await r.json()
      setImgDrafts((j.predictions||[]).map((p,i)=>({id:Date.now()+i,dataUrl:`data:${p.mimeType||'image/png'};base64,${p.bytesBase64Encoded}`})))
    ...
    <div style={{padding:'24px 28px 60px'}}>
      <Sec title="Identity" open={true}>
        <div style={{paddingTop:'8px'}}>
          ...
          <div style={{marginBottom:'12px'}}><div style={lbl}>Style Consistency Anchor</div><textarea {...f('styleconsistencyanchor')} style={{...mkT(locked),minHeight:'56px'}} placeholder="Phrase prepended to every prompt for visual consistency across all generations..."/></div>
      ...
      <Sec title="File Uploads"><div style={{paddingTop:'8px'}}>{[['Reference Image','JPG, PNG, WEBP'],['Audio File','MP3, WAV, M4A']].map(([l,a])=>(<div key={l} style={{marginBottom:'10px'}}><div style={lbl}>{l}</div><div style={{border:`1px dashed rgba(201,146,74,0.2)`,padding:'14px',textAlign:'center',fontSize:'0.72rem',color:CHARCOAL,cursor:locked?'default':'pointer',opacity:locked?0.4:1}}>Click to upload ({a})</div></div>))}</div></Sec>
      ...
      <Sec title="AI Prompt" open={true}><div style={{paddingTop:'8px'}}>
        <div style={{display:'flex',gap:'10px',marginBottom:'10px',alignItems:'flex-end'}}>
          <div style={{flex:1}}><div style={lbl}>Image AI Model</div><select value={data.aiimagemodel||''} onChange={e=>!locked&&onChange('aiimagemodel',e.target.value)} disabled={locked} style={mkS(locked)}><option value="">— Default (Imagen 4) —</option>{IMG_MODELS.map(m=><option key={m}>{m}</option>)}</select></div>
          <button onClick={autoPrompt} disabled={locked} style={{background:'rgba(201,146,74,0.1)',border:`1px solid rgba(201,146,74,0.25)`,color:locked?MUTED:GOLD,padding:'9px 14px',cursor:locked?'default':'pointer',fontFamily:'DM Sans, sans-serif',fontSize:'0.68rem',letterSpacing:'0.08em',textTransform:'uppercase',whiteSpace:'nowrap'}}>Auto-Prompt</button>
        </div>
      ...
      <Sec title="Image Generation" open={true}><div style={{paddingTop:'8px'}}>
        <button onClick={genImages} disabled={genImg||locked} style={{background:genImg?'rgba(201,146,74,0.4)':GOLD,border:'none',color:SURFACE,padding:'9px 20px',cursor:genImg||locked?'default':'pointer',fontFamily:'DM Sans, sans-serif',fontSize:'0.72rem',letterSpacing:'0.1em',textTransform:'uppercase',fontWeight:500,marginBottom:'12px'}}>
        ...
        {imgDrafts.length>0&&<div style={{display:'grid',gridTemplateColumns:`repeat(${Math.min(imgDrafts.length,3)},1fr)`,gap:'8px'}}>
          {imgDrafts.map((d,i)=>(<div key={d.id}><div onClick={()=>{setFinalIdx(i);onChange('finalimage',d.dataUrl)}} style={{aspectRatio:'1',background:'#000',border:`2px solid ${finalIdx===i?GOLD:BORDER}`,cursor:'pointer',overflow:'hidden',position:'relative'}}><img src={d.dataUrl} alt={`var ${i+1}`} style={{width:'100%',height:'100%',objectFit:'cover'}}/>{finalIdx===i&&<div style={{position:'absolute',top:'6px',right:'6px',background:GOLD,color:SURFACE,width:'18px',height:'18px',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.65rem',fontWeight:700}}>✓</div>}</div><button onClick={()=>{setFinalIdx(i);onChange('finalimage',d.dataUrl)}} style={{width:'100%',marginTop:'4px',background:finalIdx===i?'rgba(201,146,74,0.12)':'none',border:`1px solid ${finalIdx===i?'rgba(201,146,74,0.35)':BORDER}`,color:finalIdx===i?GOLD:CHARCOAL,padding:'5px 0',cursor:'pointer',fontSize:'0.63rem',letterSpacing:'0.08em',textTransform:'uppercase',fontFamily:'DM Sans, sans-serif'}}>{finalIdx===i?'✓ Final':'Select'}</button></div>))}
        </div>}
      </div></Sec>
    </div>
  )
...
export default function AssetDetail(){
  ...
  return(
    <div style={{fontFamily:'DM Sans, sans-serif',color:CREAM,background:SURFACE,minHeight:'100vh'}}>
      ...
      {activeInst&&activeTab==='voice'&&showVoice&&<VoiceTab data={activeInst} onChange={upd} locked={locked} assetMeta={assetMeta} savedAssetId={savedAssetId} onVoiceIdChange={async(id)=>{setAssetMeta(m=>({...m,elevenlabs_voice_id:id}));if(savedAssetId){await supabase.from('assets').update({elevenlabs_voice_id:id,updatedate:new Date().toISOString()}).eq('assetid',savedAssetId);toast_('Voice ID saved ✓')}}}/>}
    </div>
  )
}
