...
function DimensionRow({ dim, score }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: `1px solid ${BORDER}` }}>
      ...
      <div style={{ width: 32, textAlign: 'right', fontFamily: 'Cormorant Garamond, serif', fontSize: '0.9rem', color, flexShrink: 0 }}>
        {score !== undefined ? `${score}` : '—'}<span style={{ color: MUTED, fontSize: '0.65rem' }}>/{dim.max}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '0 28px 28px' }}>
      <div style={{ textAlign: 'center', padding: '28px 0 24px', borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '4rem', color: v.color, fontWeight: 700, lineHeight: 1 }}>{title.score}</div>
        <div style={{ color: MUTED, fontSize: '0.7rem', letterSpacing: '0.1em', marginTop: 4 }}>OUT OF 60</div>
        <div style={{ display: 'inline-block', background: v.bg, color: v.color, padding: '5px 14px', borderRadius: 2, fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 10, fontWeight: 600 }}>{v.label}</div>
        {title.platform && (
      ...
      <div style={{ display: 'flex', gap: 12, padding: '20px 0', borderBottom: `1px solid ${BORDER}` }}>
        {Object.entries(PILLAR_INFO).map(([key, info]) => {
          const ps = key === 'A' ? title.pillarA : key === 'B' ? title.pillarB : title.pillarC
          const pct = ps !== null ? (ps / info.max) * 100 : 0
          return (
            <div key={key} style={{ flex: 1, background: '#1E1A14', border: `1px solid ${BORDER}`, borderRadius: 4, padding: '12px 14px' }}>
              <div style={{ fontSize: '0.62rem', color: MUTED, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 6 }}>Pillar {key}</div>
              <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.3rem', color: info.color, fontWeight: 700 }}>{ps}<span style={{ color: MUTED, fontSize: '0.8rem' }}>/{info.max}</span></div>
      ...
      {['A', 'B', 'C'].map(pillar => (
        <div key={pillar} style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '0.75rem', color: PILLAR_INFO[pillar].color, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 600 }}>Pillar {pillar} — {PILLAR_INFO[pillar].label}</span>
      ...
      <div style={{ marginTop: 24, padding: 16, background: '#1A1810', border: `1px solid ${BORDER}`, borderRadius: 4 }}>
        <div style={{ fontSize: '0.65rem', color: MUTED, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>Score Thresholds (v2.2)</div>
        {[
          ...
          <div key={t.range} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0' }}>
            ...
            <span style={{ width: 70, color: t.color, fontSize: '0.68rem', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{t.label}</span>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
        <button onClick={onClose}
          style={{ flex: 1, background: GOLD, border: 'none', color: '#1A1810', padding: '11px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 2 }}>
          Open in Writers Room →
        </button>
        <button onClick={onClose}
          style={{ background: 'none', border: `1px solid ${BORDER}`, color: CHARCOAL, padding: '11px 18px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', cursor: 'pointer', borderRadius: 2 }}>
          Close
        </button>
      </div>
    </div>
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      <div style={{ height: 130, background: 'linear-gradient(160deg, #1E1A14, #0D0B08)',
        ...
        <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2.8rem', color: 'rgba(201,146,74,0.25)', fontWeight: 700, letterSpacing: 2 }}>{initials}</div>
        <div style={{ position: 'absolute', top: 10, right: 10, background: `${sc.color}22`, border: `1px solid ${sc.color}44`, color: sc.color, padding: '2px 8px', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', borderRadius: 2 }}>
          {sc.label}
        </div>
        {/* R2 indicator */}
        {title.hasManuscript && (
          <div style={{ position: 'absolute', bottom: 8, left: 10, background: 'rgba(122,158,200,0.15)', color: BLUE, padding: '2px 7px', fontSize: '0.58rem', borderRadius: 2 }}>
            📄 R2
          </div>
        )}
      </div>
      <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.05rem', color: CREAM, fontWeight: 400, lineHeight: 1.3, marginBottom: 3 }}>{title.title}</div>
          <div style={{ fontSize: '0.7rem', color: MUTED, letterSpacing: '0.06em' }}>{title.genre || 'Unclassified'}</div>
        </div>
        {title.tropes && title.tropes.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {title.tropes.map((t, i) => (
              <span key={t} style={{ background: TROPE_COLORS[i % TROPE_COLORS.length], color: CREAM, fontSize: '0.62rem', padding: '2px 7px', borderRadius: 2, letterSpacing: '0.04em' }}>{t}</span>
            ))}
          </div>
        )}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', paddingTop: 10, borderTop: `1px solid ${BORDER}` }}>
...
function TitlesGrid({ titles, loading, onNewManuscript, onOpenTitle }) {
  const [search, setSearch] = useState('')
  const filtered = titles.filter(t =>
    t.title.toLowerCase().includes(search.toLowerCase()) ||
    (t.genre || '').toLowerCase().includes(search.toLowerCase())
  )
  const scoredCount = titles.filter(t => t.score !== null && t.score !== undefined).length
    ...
    <div style={{ padding: '32px 40px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: MUTED, fontSize: '0.78rem', margin: '6px 0 0', letterSpacing: '0.04em' }}>
            {titles.length} titles · {scoredCount} scored
          </p>
        </div>
        <button onClick={onNewManuscript}
          style={{ background: GOLD, border: 'none', color: '#1A1810', padding: '10px 20px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 600, borderRadius: 2 }}>
          + New Manuscript
        </button>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search titles, genres…"
          style={{ background: SURFACE2, border: `1px solid ${BORDER}`, color: CREAM, padding: '9px 14px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.82rem', outline: 'none', width: 280, borderRadius: 2 }} />
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
          ...
          <div onClick={onNewManuscript}
            style={{ background: 'transparent', border: `1px dashed ${BORDER}`, borderRadius: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', minHeight: 280, gap: 10, opacity: 0.6, transition: 'opacity 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.opacity = 1}
            onMouseLeave={e => e.currentTarget.style.opacity = 0.6}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', border: `1px solid ${CHARCOAL}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: CHARCOAL, fontSize: '1.4rem' }}>+</div>
            <span style={{ color: CHARCOAL, fontSize: '0.72rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Add Manuscript</span>
          </div>
        </div>
      )}
    </div>
...
function ImportWizard({ onClose, onComplete, userId }) {
  ...
  function handleAgreementDrop(e) {
    e.preventDefault()
    const f = e.dataTransfer?.files[0] || e.target.files[0]
    if (f) setAgreementFile(f)
  }
  ...
  async function handleBeginProcessing() {
    ...
    try {
      // Stage 0: Upload manuscript to R2
      ...
      // Stage 1: Create production record in Supabase
      setProcessingStage(1)
      const { data: production, error: prodErr } = await supabase
        .from('productions')
        .insert({
          productiontitle: form.name,
      ...
      setCreatedTitle(production)

      // Stage 1b: Upload agreement if provided
      if (agreementFile && userId) {
        await r2.uploadFile(userId, titleSlug, 'legal-documents', agreementFile, {
          type: form.publicDomain ? 'public-domain' : 'license-agreement',
          signDate: form.signDate || '',
        })
      }

      // Stages 2-5: AI processing (mock for now — will be replaced with actual AI pipeline)
      for (let i = 2; i < PROCESSING_STAGES.length; i++) {
      ...
      await supabase
        ...
        .eq('productionid', production.productionid)
  ...
  const lbl = { display: 'block', fontSize: '0.65rem', color: CHARCOAL, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 6 }
  const inp = { width: '100%', background: SURFACE2, border: `1px solid ${BORDER}`, color: CREAM, padding: '10px 12px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.83rem', outline: 'none', boxSizing: 'border-box', borderRadius: 2 }

  // Mock result for score report (will be replaced with AI-generated scores)
  const MOCK_RESULT = {
    id: createdTitle?.productionid || 99,
    ...
    agreement: !!form.signDate || form.publicDomain,
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
      onClick={onClose}>
      <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, width: 640, maxWidth: '95vw', maxHeight: '92vh', overflowY: 'auto', borderRadius: 4 }}
        ...
        <div style={{ padding: '22px 28px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ color: MUTED, fontSize: '0.7rem', margin: '4px 0 0', letterSpacing: '0.04em' }}>
              AI-powered 4-step scoring pipeline · v2.2 · R2 Storage
            </p>
          </div>
        ...
        <div style={{ padding: '14px 28px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center' }}>
          {STEPS.map((s, i) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : 'none' }}>
              ...
              {i < STEPS.length - 1 && (
                <div style={{ flex: 1, height: 1, background: i < step ? GREEN : BORDER, margin: '0 12px', transition: 'background 0.3s' }} />
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '24px 28px' }}>
          ...
          {step === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div onDragOver={e => { e.preventDefault(); setDragOver(true) }}
                ...
                style={{ border: `1px dashed ${dragOver ? GOLD : BORDER}`, borderRadius: 4, padding: '28px 20px', textAlign: 'center', cursor: 'pointer', background: dragOver ? 'rgba(201,146,74,0.05)' : 'transparent', transition: 'all 0.2s' }}
                ...
                {file ? (
                  <div>
                    ...
                    <div style={{ color: MUTED, fontSize: '0.7rem', marginTop: 3 }}>{(file.size / 1024 / 1024).toFixed(1)} MB</div>
                    <div style={{ color: BLUE, fontSize: '0.65rem', marginTop: 4, fontFamily: 'monospace' }}>
                      → r2://culmina-docs/{slugify(form.name || 'untitled')}/manuscripts/
                    </div>
                  </div>
                ) : (
                  <div>
              ...
              <button onClick={() => setStep(1)} disabled={!form.name}
                style={{ background: form.name ? GOLD : 'rgba(201,146,74,0.3)', border: 'none', color: form.name ? '#1A1810' : MUTED, padding: '12px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, cursor: form.name ? 'pointer' : 'not-allowed', borderRadius: 2 }}>
                Continue →
              </button>
            </div>
          )}
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '12px 14px', border: `1px solid ${form.publicDomain ? GREEN : BORDER}`, borderRadius: 4, background: form.publicDomain ? 'rgba(74,156,122,0.08)' : 'transparent', transition: 'all 0.2s' }}>
                ...
                <div>
                  ...
                  <div style={{ color: MUTED, fontSize: '0.7rem', marginTop: 2 }}>Skip rights fields — auto-scores IP Clarity: 3</div>
                </div>
              </label>
              ...
              {!form.publicDomain && (
                <>
                  ...
                  {/* Agreement upload — goes to R2 legal-documents/ */}
                  <div
                    onDragOver={e => e.preventDefault()}
                    onDrop={handleAgreementDrop}
                    onClick={() => document.getElementById('agreement-file-input').click()}
                    style={{ border: `1px dashed ${agreementFile ? GREEN : BORDER}`, borderRadius: 4, padding: '18px', textAlign: 'center', cursor: 'pointer', background: agreementFile ? 'rgba(74,156,122,0.04)' : 'transparent' }}>
                    <input id="agreement-file-input" type="file" accept=".pdf" style={{ display: 'none' }} onChange={handleAgreementDrop} />
                    {agreementFile ? (
                      <div>
                        <div style={{ color: GREEN, fontSize: '0.83rem' }}>✓ {agreementFile.name}</div>
                        <div style={{ color: BLUE, fontSize: '0.65rem', marginTop: 4, fontFamily: 'monospace' }}>
                          → r2://culmina-docs/{slugify(form.name || 'untitled')}/legal-documents/
                        </div>
                      </div>
                    ) : (
                      <div>
                        <div style={{ color: CHARCOAL, fontSize: '0.78rem' }}>Drop signed agreement PDF</div>
                        <div style={{ color: MUTED, fontSize: '0.68rem', marginTop: 4 }}>PDF · max 10 MB · stored in R2 legal-documents/</div>
                      </div>
                    )}
                  </div>
                </>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button onClick={() => setStep(0)}
                  style={{ background: 'none', border: `1px solid ${BORDER}`, color: CHARCOAL, padding: '11px 20px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', cursor: 'pointer', borderRadius: 2 }}>← Back</button>
                <button onClick={() => setStep(2)}
                  style={{ flex: 1, background: GOLD, border: 'none', color: '#1A1810', padding: '11px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 2 }}>Continue →</button>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              {!processing && (
                <div style={{ background: '#1A1810', border: `1px solid ${BORDER}`, borderRadius: 4, padding: 16 }}>
                  ...
                  {[
                    ...
                    ['Language', form.language],
                    ['File', file ? `${file.name} (${(file.size / 1024 / 1024).toFixed(1)} MB)` : 'No file selected'],
                    ['Rights', form.publicDomain ? 'Public Domain' : form.signDate ? `Agreement signed ${form.signDate}` : 'Agreement pending'],
                    ['Agreement', agreementFile ? agreementFile.name : '—'],
                    ['Source URL', form.url || '—'],
                    ['R2 Path', `/${slugify(form.name)}/manuscripts/`],
                  ].map(([k, v]) => (
              ...
              {!processing && (
                <div style={{ background: '#1A1810', border: `1px solid ${BORDER}`, borderRadius: 4, padding: 16 }}>
                  <div style={{ fontSize: '0.65rem', color: MUTED, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>AI Scoring Pipeline (v2.2)</div>
              ...
              {processing && (
                <div style={{ padding: '20px 0', textAlign: 'center' }}>
                  {PROCESSING_STAGES.map((stage, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', opacity: i <= processingStage ? 1 : 0.25, transition: 'opacity 0.4s' }}>
                      ...
                      <span style={{ color: i === processingStage ? CREAM : MUTED, fontSize: '0.78rem', textAlign: 'left' }}>{stage}</span>
                      {/* Upload progress for stage 0 */}
                      {i === 0 && processingStage === 0 && uploadProgress > 0 && uploadProgress < 1 && (
                        <span style={{ color: GOLD, fontSize: '0.7rem', marginLeft: 'auto' }}>{Math.round(uploadProgress * 100)}%</span>
                      )}
                    </div>
                  ))}
                  ...
                  {!done && (
                    <div style={{ marginTop: 16, height: 3, background: 'rgba(255,255,255,0.06)', borderRadius: 2 }}>
                      <div style={{ width: `${(processingStage / (PROCESSING_STAGES.length - 1)) * 100}%`, height: '100%', background: GOLD, borderRadius: 2, transition: 'width 0.6s ease' }} />
                    </div>
                  )}

                  {done && (
                    <div style={{ marginTop: 20, padding: '12px 16px', background: 'rgba(74,156,122,0.12)', border: '1px solid rgba(74,156,122,0.3)', borderRadius: 4, textAlign: 'left' }}>
                      <div style={{ color: GREEN, fontSize: '0.83rem', fontWeight: 600 }}>✓ Processing complete</div>
                      <div style={{ color: CHARCOAL, fontSize: '0.72rem', marginTop: 4 }}>
                        Manuscript uploaded to R2 · Production record created
                      </div>
                      <button onClick={() => setStep(3)}
                        style={{ marginTop: 12, background: GOLD, border: 'none', color: '#1A1810', padding: '9px 18px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.72rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 2 }}>
                        View Score Report →
                      </button>
                    </div>
                  )}
              ...
              {uploadError && (
                <div style={{ padding: '10px 14px', background: 'rgba(200,75,49,0.1)', border: '1px solid rgba(200,75,49,0.2)', color: RED, fontSize: '0.78rem', borderRadius: 4 }}>
                  ⚠ {uploadError}
                </div>
              )}
          ...
          {/* STEP 4 — Score Report */}
          {step === 3 && (
            <ScoreReport title={MOCK_RESULT} onClose={() => { onComplete(); onClose() }} />
          )}
        </div>
      </div>
    </div>
...
function TitleDetailDrawer({ title, onClose }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
      onClick={onClose}>
      <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, width: 620, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', borderRadius: 4 }}
        onClick={e => e.stopPropagation()}>

        <div style={{ padding: '22px 28px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              <span style={{ background: (STATUS_CONFIG[title.status]?.color || BLUE) + '22', border: `1px solid ${(STATUS_CONFIG[title.status]?.color || BLUE)}44`, color: STATUS_CONFIG[title.status]?.color || BLUE, padding: '2px 8px', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', borderRadius: 2 }}>
                {STATUS_CONFIG[title.status]?.label || title.status}
              </span>
              {title.platform && <span style={{ color: CHARCOAL, fontSize: '0.68rem' }}>{title.platform}</span>}
              {title.hasManuscript && <span style={{ color: BLUE, fontSize: '0.62rem' }}>📄 In R2</span>}
            </div>
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.6rem', color: CREAM, fontWeight: 300, margin: 0 }}>{title.title}</h2>
            <p style={{ color: MUTED, fontSize: '0.72rem', margin: '4px 0 0' }}>
              {title.genre || 'Unclassified'} · v{title.version || 1} · {title.language || 'English'}
            </p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: MUTED, cursor: 'pointer', fontSize: '1.2rem', padding: 4 }}>✕</button>
        </div>
        ...
        {title.score !== null && title.score !== undefined ? (
          ...
          <div style={{ padding: '32px 28px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={onClose}
                style={{ background: GOLD, border: 'none', color: '#1A1810', padding: '10px 20px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 2 }}>
                {title.status === 'error' ? 'Retry Processing' : 'Run Scoring'}
              </button>
              <button onClick={onClose}
                style={{ background: 'none', border: `1px solid ${BORDER}`, color: CHARCOAL, padding: '10px 18px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', cursor: 'pointer', borderRadius: 2 }}>Close</button>
            </div>
          </div>
        )}
      </div>
...
export default function Manuscript() {
  ...
  async function loadTitles() {
    ...
    try {
      ...
      // Map to UI shape and check R2 for manuscripts
      const mapped = await Promise.all((data || []).map(async (p) => {
        const slug = slugify(p.productiontitle)
        let hasManuscript = false

        // Check if manuscripts folder has files in R2
        if (userId) {
          try {
            const result = await r2.listFiles(userId, slug, 'manuscripts')
            hasManuscript = result.objects && result.objects.length > 0
          } catch {
            // No files yet — that's fine
          }
        }

        return {
          id: p.productionid,
          title: p.productiontitle,
          slug,
          genre: '',
          tropes: [],
          status: p.productionstatus || 'uploaded',
          episodes: null,
          score: null,
          platform: null,
          version: 1,
          ...
          pillarA: null, pillarB: null, pillarC: null,
          dimensions: null,
          hasManuscript,
        }
      }))
  ...
  function handleComplete() {
    loadTitles() // Refresh from Supabase + R2
  }
    ...
    <div style={{ minHeight: '100vh', background: SURFACE2, fontFamily: 'DM Sans, sans-serif', color: CREAM }}>
      ...
      {showWizard && (
        <ImportWizard
          ...
          onComplete={() => { handleComplete(); setShowWizard(false) }}
        />
      )}
