...
const LEVEL_COLORS = {
  TITLE: GOLD, ARC: '#9C7AC8', ACT: '#7A9EC8',
  EPISODE: '#4A9C7A', SHOT: '#C87A4A', TAKE: CHARCOAL,
}
const CHILD_MAP = {
    ...
    <div>
      ...
      >
        <span
          ...
          style={{ color: MUTED, fontSize: '0.6rem', width: '12px', display: 'inline-block', cursor: hasChildren ? 'pointer' : 'default', transform: isExpanded ? 'rotate(90deg)' : 'none', transition: 'transform 0.15s' }}>
          {hasChildren ? '▶' : ''}
        </span>
      ...
      {isExpanded && CHILD_MAP[node.productiongroup] && (
        <div
...
function DetailPanel({ node, onSave, onRefresh }) {
  const [form, setForm] = useState({})
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)

  useEffect(() => {
    if (node) setForm({ productiontitle: node.productiontitle || '', synopsis: node.synopsis || '', notes: node.notes || '', productionstatus: node.productionstatus || 'development' })
  }, [node])

  async function handleSave() {
    setSaving(true)
    await supabase.from('productions').update({ ...form, updatedate: new Date().toISOString() }).eq('productionid', node.productionid)
  ...
  if (!node) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: MUTED, fontFamily: 'Cormorant Garamond, serif', fontSize: '1.2rem', flexDirection: 'column', gap: '12px' }}>
  ...
  const color = LEVEL_COLORS[node.productiongroup] || CHARCOAL
  const labelStyle = { display: 'block', fontSize: '0.7rem', color: CHARCOAL, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '6px' }
  const inputStyle = { width: '100%', background: 'rgba(255,255,255,0.03)', border: `1px solid ${BORDER}`, color: CREAM, padding: '10px 14px', fontSize: '0.85rem', fontFamily: 'DM Sans, sans-serif', outline: 'none', marginBottom: '20px' }
  ...
  return (
    <div style={{ padding: '32px', overflowY: 'auto', height: '100%' }}>
      ...
      <input value={form.productiontitle || ''} onChange={e => setForm(f => ({ ...f, productiontitle: e.target.value }))} style={inputStyle} />

      <label style={labelStyle}>Synopsis</label>
      <textarea value={form.synopsis || ''} onChange={e => setForm(f => ({ ...f, synopsis: e.target.value }))}
        rows={4} style={{ ...inputStyle, resize: 'vertical', fontFamily: 'DM Sans, sans-serif' }} />

      <label style={labelStyle}>Status</label>
      <select value={form.productionstatus || ''} onChange={e => setForm(f => ({ ...f, productionstatus: e.target.value }))}
        style={{ ...inputStyle, cursor: 'pointer' }}>
        {['development','pre_production','in_production','post_production','distribution','completed','paused'].map(s => (
      ...
      <textarea value={form.notes || ''} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
        rows={3} style={{ ...inputStyle, resize: 'vertical', fontFamily: 'DM Sans, sans-serif' }} />

      <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
        <button onClick={handleSave} disabled={saving}
          style={{ background: saved ? '#4A9C7A' : GOLD, border: 'none', color: '#1A1810', padding: '10px 28px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500 }}>
          {saving ? 'Saving...' : saved ? 'Saved ✓' : 'Save Changes'}
        </button>
        <button style={{ background: 'none', border: `1px solid ${BORDER}`, color: CHARCOAL, padding: '10px 20px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          Create Prompt
        </button>
      </div>
    </div>
...
export default function Development() {
  ...
  async function handleAddChild(parent) {
    setAddParent(parent); setNewTitle(''); setShowAddModal(true)
  }
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 84px)', fontFamily: 'DM Sans, sans-serif', margin: '-32px', overflow: 'hidden' }}>
      ...
      <div style={{ width: '260px', minWidth: '260px', background: SURFACE2, borderRight: `1px solid ${BORDER}`, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '16px 12px', borderBottom: `1px solid ${BORDER}` }}>
          <select value={selectedTitleId} onChange={e => { setSelectedTitleId(e.target.value); setSelectedNode(null) }}
            style={{ width: '100%', background: SURFACE, border: `1px solid ${BORDER}`, color: CREAM, padding: '8px 12px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.8rem', outline: 'none', cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
          {loading ? (
            ...
            <div style={{ padding: '24px 16px', color: MUTED, fontSize: '0.78rem', textAlign: 'center', lineHeight: 1.6 }}>
              {selectedTitleId ? 'No nodes yet. Create your first Arc.' : 'Select a title to begin.'}
            </div>
          ) : tree.map(node => (
        ...
        {selectedTitleId && (
          <div style={{ padding: '12px', borderTop: `1px solid ${BORDER}` }}>
            <button onClick={() => handleAddChild({ productionid: parseInt(selectedTitleId), productiongroup: 'TITLE' })}
              style={{ width: '100%', background: 'rgba(201,146,74,0.08)', border: `1px solid ${BORDER}`, color: GOLD, padding: '8px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', letterSpacing: '0.08em' }}>
              + Add Arc
            </button>
          </div>
        )}
      ...
      <div style={{ flex: 1, background: SURFACE, overflow: 'hidden' }}>
        <DetailPanel node={selectedNode} onRefresh={loadTree} />
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, padding: '32px', width: '400px' }} onClick={e => e.stopPropagation()}>
            ...
            <input value={newTitle} onChange={e => setNewTitle(e.target.value)}
              ...
              autoFocus
              style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: `1px solid ${BORDER}`, color: CREAM, padding: '10px 14px', fontSize: '0.85rem', fontFamily: 'DM Sans, sans-serif', outline: 'none', marginBottom: '20px' }} />
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button onClick={() => setShowAddModal(false)}
                style={{ background: 'none', border: `1px solid ${BORDER}`, color: CHARCOAL, padding: '10px 20px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem' }}>
                Cancel
              </button>
            </div>
          </div>
