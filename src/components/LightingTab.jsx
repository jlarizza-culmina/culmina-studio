...
const BLANK = {
  name: '', nvvalue: '', description: '', mood: '', usecase: '', promptsnippet: '', sortorder: 99,
}

export default function LightingTab() {
  const [rows,    setRows]    = useState([])
  const [loading, setLoading] = useState(true)
  const [editing, setEditing] = useState(null)   // lightingid being edited
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('lighting').select('*').order('sortorder')
    setRows(data || [])
    setLoading(false)
  }
  ...
  const handleEdit = (row) => { setEditing(row.lightingid); setEditForm({ ...row }) }

  const handleSave = async () => {
    setSaving(true)
    const { error } = await supabase.from('lighting')
      .update({ ...editForm, updatedate: new Date().toISOString() })
      .eq('lightingid', editing)
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    const { error } = await supabase.from('lighting').insert([{ ...addForm, activestatus: 'A' }])
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this lighting type?')) return
    await supabase.from('lighting').delete().eq('lightingid', id)
    await load()
  }

  const F = ({ label, k, form, setForm, textarea, mono, width }) => (
  ...
  return (
    <div style={{ color: CREAM, fontFamily: 'DM Sans, sans-serif', fontSize: '0.82rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div style={{ fontSize: '0.72rem', color: MUTED }}>{rows.length} lighting types</div>
        <button style={btn('primary')} onClick={() => setAdding(a => !a)}>
          {adding ? 'Cancel' : '+ Add Lighting Type'}
        </button>
      </div>
      ...
      {adding && (
        <div style={{ background: SURFACE2, border: `1px solid ${BORDER}`, borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
          <div style={{ fontSize: '0.7rem', color: GOLD, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '12px', fontWeight: 600 }}>New Lighting Type</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px', gap: '12px', marginBottom: '8px' }}>
            <F label="Name" k="name" form={addForm} setForm={setAddForm} />
            <F label="Code (nvvalue)" k="nvvalue" form={addForm} setForm={setAddForm} mono />
            <F label="Sort" k="sortorder" form={addForm} setForm={setAddForm} width="60px" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '8px' }}>
            <F label="Mood keywords" k="mood" form={addForm} setForm={setAddForm} width="100%" />
            <F label="Use case" k="usecase" form={addForm} setForm={setAddForm} width="100%" />
          </div>
      ...
      {loading ? (
        ...
        <div>
          {rows.map((row, i) => (
            <div key={row.lightingid} style={{
              background: editing === row.lightingid ? '#1E1C14' : i % 2 === 0 ? SURFACE : SURFACE2,
              border: editing === row.lightingid ? `1px solid ${GOLD}44` : `1px solid ${BORDER}`,
              borderRadius: '6px', marginBottom: '6px', padding: '12px 14px',
              opacity: row.activestatus === 'A' ? 1 : 0.45,
            }}>
              {editing === row.lightingid ? (
                /* Edit mode */
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px', gap: '12px', marginBottom: '8px' }}>
                    <F label="Name" k="name" form={editForm} setForm={setEditForm} />
                    <F label="Code" k="nvvalue" form={editForm} setForm={setEditForm} mono />
                    <F label="Sort" k="sortorder" form={editForm} setForm={setEditForm} width="60px" />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '8px' }}>
                    <F label="Mood" k="mood" form={editForm} setForm={setEditForm} width="100%" />
                    <F label="Use case" k="usecase" form={editForm} setForm={setEditForm} width="100%" />
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '3px' }}>
                      ...
                      {row.mood && <span style={{ fontSize: '0.68rem', color: GOLD }}>{row.mood}</span>}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                    ...
                    <button onClick={() => handleDelete(row.lightingid)} style={{ ...btn(), fontSize: '0.68rem', padding: '3px 8px', color: RED }}>Delete</button>
                  </div>
                </div>
              )}
            </div>
